...
const cardOptions = [];
SUIT_VALUES.forEach((suitValue) => {
  RANK_VALUES.slice(1).forEach((rankValue) => {
    cardOptions.push(
      `${suitValue.gssCode}${rankValue.gssCode} - ${rankValue.label} of ${suitValue.label} `
    );
  });
});

const defaultOption = document.createElement('option');
defaultOption.value = 'Select a card';
defaultOption.innerHTML = 'Select a card';
cardSelect.appendChild(defaultOption);

cardOptions.forEach((cardOption) => {
  const option = document.createElement('option');
  option.value = cardOption;
  option.innerHTML = cardOption;
...
const getSignal = (card) => {
  ...
  return `*0D${code1}${code2}${checksum}`;
};
